...
const AddRecipeModal = ({ onClose, onRecipeAdded, onNutritionCalculated, initialData }) => {
  ...
  const handleShowNutrition = async e => {
    e.preventDefault()
    ...
    try {
      ...
      const payload = {
        bahan: validIngredients.map(item => ({
          bahan_id: item.bahanId,
          gramasi: parseFloat(item.gramasi)
        }))
      }

      const response = await generateNutrition(payload)
      const data = response

      console.log('Data Generate Nutrition ', data)

      // ✅ Set data for RecommendationCard
      ...
      setError('Gagal menghitung nutrisi. Coba lagi.')
    } finally {
  ...
  // ✅ NEW: Handle going back to input view
  ...
  return (
    <div className='bg-white rounded-2xl shadow-2xl w-full flex flex-col animate-in fade-in duration-200'>
      ...
      {!showRecommendation ? (
        <form onSubmit={handleShowNutrition} className='flex-1'>
